...
## 6. P2 후속 — main.js 트레이 브랜드 치환 (Reviewer 지적 해소)

- §5의 첫 잔여분을 해소: `main.js:84` 발룬 title과 `:102` 툴팁의
  `지점관리프로그램`을 `SME-ERP`로 치환. 렌더러 표시 문자열 2곳のみ,
  로직·동작 무변경. `rg 지점관리프로그램 main.js` 0건 + `node --check` 통과,
  3종 테스트 118 PASS 회귀 없음으로 검증 후 커밋.
- 남은 공개 게이트는 `build/icon.ico` 에셋과 Fresh Init 분리 2건.
